...
# Load model and data
model = mujoco.MjModel.from_xml_path("ctrls_circles_cloth19.xml")
    ...
    for i in range(10000000):
        ...
        if target_index < half_len:
            ...
            if r_flag:
                ...
                aaa = all_pairs[target_index:target_index + 1]
                print(f"Selected single step at target_index={target_index}")

        for step_pairs in aaa:
            max_step_size = 0.001
            all_reached = False

            # Precompute final diff for early stopping
            last_block = all_pairs[target_index]
            i_last, j_last = last_block[-1]
            lc_i_qpos_addr = model.joint(f'ctrl_{i_last}_x').qposadr
            lc_j_qpos_addr = model.joint(f'ctrl_{j_last}_x').qposadr
            ...
            while not all_reached:
                ...
                for (i, j) in step_pairs:
                    # Read positions
                    c_i_pos = np.array([
                        data.qpos[model.joint(f'ctrl_{i}_x').qposadr],
                        data.qpos[model.joint(f'ctrl_{i}_y').qposadr],
                        data.qpos[model.joint(f'ctrl_{i}_z').qposadr]
                    ])
                    c_j_pos = np.array([
                        data.qpos[model.joint(f'ctrl_{j}_x').qposadr],
                        data.qpos[model.joint(f'ctrl_{j}_y').qposadr],
                        data.qpos[model.joint(f'ctrl_{j}_z').qposadr]
                    ])
                    ...
                    if np.linalg.norm(diff) >= 9e-4:
                        ...
                        # Update c_i position
                        data.qpos[model.joint(f'ctrl_{i}_x').qposadr] += step[0]
                        data.qpos[model.joint(f'ctrl_{i}_y').qposadr] += step[1]
                        data.qpos[model.joint(f'ctrl_{i}_z').qposadr] += step[2]

                        # Zero velocity for each axis
                        for axis in ['x', 'y', 'z']:
                            qvel_addr = model.joint(f'ctrl_{i}_{axis}').dofadr
                            data.qvel[qvel_addr] = 0
                ...
                # Final block check
                lc_i_pos = np.array([
                    data.qpos[lc_i_qpos_addr],
                    data.qpos[model.joint(f'ctrl_{i_last}_y').qposadr],
                    data.qpos[model.joint(f'ctrl_{i_last}_z').qposadr]
                ])
                lc_j_pos = np.array([
                    data.qpos[lc_j_qpos_addr],
                    data.qpos[model.joint(f'ctrl_{j_last}_y').qposadr],
                    data.qpos[model.joint(f'ctrl_{j_last}_z').qposadr]
                ])
