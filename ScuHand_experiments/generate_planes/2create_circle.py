...
def generate_clipped_concentric_circle_mesh(radius=0.05, rings=5, segments=64, clip_degrees=10, first_ring_radius=0.025):
    vertices = []
    faces = []
    ...
    ring_radii = [first_ring_radius]
    if rings > 1:
        remaining_rings = rings - 1
        # Linearly interpolate from first_ring_radius to outer radius
        ring_radii += list(np.linspace(first_ring_radius + (radius - first_ring_radius)/remaining_rings, radius, remaining_rings))

    # Generate concentric ring vertices
    for r_idx, curr_radius in enumerate(ring_radii):
...
# Example usage
generate_clipped_concentric_circle_mesh(radius=0.05, rings=4, segments=18, clip_degrees=0.1, first_ring_radius=0.02)
