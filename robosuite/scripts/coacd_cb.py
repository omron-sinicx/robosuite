...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="output model path. For single file, includes extension. For multiple files, used as the folder name and prefix.",
    )
    parser.add_argument(
        "-m",
        "--mode",
        ...
        default='single',
        help="'single' for one file with all parts, 'multiple' for one file per part"
    )
    ...
    parser.add_argument(
        ...
        "--extrude",
        action="store_true",
        help="If extrude is enabled, extrude the neighboring convex hulls along the overlap face.",
    )
    parser.add_argument(
        "-em",
        "--extrude-margin",
    ...
    if args.mode == 'single':
        # Create a scene with all parts (with random colors as in original)
        scene = trimesh.Scene()
        np.random.seed(0)
        for p in mesh_parts:
        ...
        if not ext:
            ext = '.stl'  # Default to STL if no extension provided

        for i, part in enumerate(mesh_parts):
            part_file = f"{folder_name}/{base_name}_part_{i}{ext}"
            part.export(part_file)
            if not args.quiet:
                print(f"Saved part {i} to {part_file}")
